...
# IMAGE RESIZE (STABLE)
# -----------------------------
def resize_image(data: bytes, max_size: int, content_type: str) -> bytes:
    ...
    if "jpeg" in content_type:
        out = resized.jpegsave_buffer(Q=85)
    elif "png" in content_type:
        out = resized.pngsave_buffer()
    elif "webp" in content_type:
        out = resized.webpsave_buffer(Q=85)
    else:
        return data
...
def response(flow: http.HTTPFlow):
    ...
    try:
        ...
        resize_cfg = cfg.get("img_resize", {})
        enabled = resize_cfg.get("enabled", False)
        max_size = resize_cfg.get("max_size", 1000)
        ...
        if enabled:
            ...
            cache_file = cache_path(key, f"resized_{max_size}x{max_size}.{ext}")

            # ---- CACHE HIT (already resized) ----
            if os.path.exists(cache_file):
                ...
                flow.response = http.Response.make(
                    ...
                    cached,
                    {
                        ...
                        "X-Cache": "HIT_RESIZED",
                        "X-Resized-To": f"{max_size}x{max_size}",
                    },
                )

                log_event("HIT_RESIZED", url, key, cache_file)
            ...
            # ---- CACHE MISS ----
            resized = resize_image(original, max_size, ct)
            ...
            flow.response = http.Response.make(
                ...
                resized,
                {
                    ...
                    "Cache-Control": "no-store",
                    "X-Cache": "MISS_RESIZED",
                    "X-Resized-To": f"{max_size}x{max_size}",
                },
            )

            log_event("MISS_RESIZED", url, key, cache_file)
